// Obol Purse: the first add-on of Obol, a loadable Hytale plugin of its own
...
dependencies {
    ...
    // Obol, for this project's own server (`:addons:purse:runServer`).
...
// src/main/resources, so purse.json (the config, written with its defaults
// on first start) lands there. Never ship it.
tasks.named<ProcessResources>("processResources") {
    exclude("purse.json", "purse.json.bak")
}
